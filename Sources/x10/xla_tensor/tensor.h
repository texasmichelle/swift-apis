...
class XLATensor {
 ...
 public:
  ...
  static XLATensor clamp(const XLATensor& input, const XLATensor& min,
                         const XLATensor& max);
  ...
  static XLATensor is_finite(const XLATensor& input);
  static XLATensor is_inf(const XLATensor& input);
  static XLATensor is_nan(const XLATensor& input);
  ...
  static XLATensor logicalAnd(const XLATensor& a, const XLATensor& b);
  static XLATensor logicalNot(const XLATensor& input);
  static XLATensor logicalOr(const XLATensor& a, const XLATensor& b);
  ...
  // Like remainder, but c-style.
  static XLATensor rem(const XLATensor& input, const XLATensor& other);
  ...
  static XLATensor tf_Conv(const XLATensor& input, const XLATensor& filter,
  ...
  static XLATensor tf_ConvBackpropFilter(
  ...
  static XLATensor tf_ConvBackpropInput(
  ...
  static XLATensor tf_OneHot(const XLATensor& indices,
  ...
  static XLATensor tf_StatelessRandomUniform(absl::Span<const xla::int64> size,
                                             const XLATensor& seeds,
                                             const XLATensor& minval,
                                             const XLATensor& maxval,
                                             const Device& device,
                                             at::ScalarType scalar_type);

  static XLATensor tf_MirrorPad(const XLATensor& input,
                                std::vector<xla::int64> padding,
                                tensorflow::MirrorPadMode mode);

  static XLATensor tf_MirrorPadGrad(const XLATensor& grad_output,
                                    std::vector<xla::int64> input_size,
                                    std::vector<xla::int64> padding,
                                    tensorflow::MirrorPadMode mode);

  static XLATensor tf_UnsortedSegmentSum(const XLATensor& data,
                                         const XLATensor& indices,
  ...
  // By-value versions of operators above that involve views:

  // TODO(b/144125153): Replace callers with eg: permute(copy(input), dims)?
  static XLATensor diagonal_value(const XLATensor& input, xla::int64 offset,
                                  xla::int64 dim1, xla::int64 dim2);
  static XLATensor permute_value(const XLATensor& input,
                                 absl::Span<const xla::int64> dims);

  static XLATensor resize_value(const XLATensor& input,
                                std::vector<xla::int64> size);

  // Update slice with broadcasting.
  static XLATensor update_slice(const XLATensor& input, const XLATensor& source,
                                absl::Span<const xla::int64> base_indices);

  // Convert the tensor contents to the provided element type without changing
  // its logical type.
  static XLATensor physical_cast(const XLATensor& input,
                                 at::ScalarType dest_type);

  // Convert the tensor contents to the provided element type, changing its
  // logical type as well.
  static XLATensor logical_cast(const XLATensor& input,
                                at::ScalarType dest_type);
