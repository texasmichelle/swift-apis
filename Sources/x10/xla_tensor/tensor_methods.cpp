...
#include "tensorflow/compiler/tf2xla/xla_tensor/ops/index_select.h"
...
#include "tensorflow/compiler/tf2xla/xla_tensor/ops/l1_loss_backward.h"
...
#include "tensorflow/compiler/tf2xla/xla_tensor/ops/logsumexp.h"
...
#include "tensorflow/compiler/tf2xla/xla_tensor/ops/native_batch_norm_backward.h"
...
#include "tensorflow/compiler/tf2xla/xla_tensor/ops/put.h"
#include "tensorflow/compiler/tf2xla/xla_tensor/ops/qr.h"
...
#include "tensorflow/compiler/tf2xla/xla_tensor/ops/svd.h"
...
#include "tensorflow/compiler/tf2xla/xla_tensor/tensor_ops.h"
...
XLATensor XLATensor::clamp(const XLATensor& input, const XLATensor& min,
                           const XLATensor& max) {
  return input.CreateFrom(
      ir::ops::Clamp(input.GetIrValue(), min.GetIrValue(), max.GetIrValue()));
}
...
XLATensor XLATensor::is_finite(const XLATensor& input) {
  return input.CreateFrom(ir::ops::IsFinite(input.GetIrValue()),
                          at::ScalarType::Bool);
}

XLATensor XLATensor::is_inf(const XLATensor& input) {
  return input.CreateFrom(ir::ops::IsInf(input.GetIrValue()),
                          at::ScalarType::Bool);
}

XLATensor XLATensor::is_nan(const XLATensor& input) {
...
XLATensor XLATensor::logicalAnd(const XLATensor& a, const XLATensor& b) {
  return a.CreateFrom(ir::ops::LogicalAnd(a.GetIrValue(), b.GetIrValue()));
}

XLATensor XLATensor::logicalNot(const XLATensor& input) {
  return input.CreateFrom(ir::ops::Not(input.GetIrValue()));
}

XLATensor XLATensor::logicalOr(const XLATensor& a, const XLATensor& b) {
  return a.CreateFrom(ir::ops::LogicalOr(a.GetIrValue(), b.GetIrValue()));
}
...
XLATensor XLATensor::tf_MirrorPad(const XLATensor& input,
                                  std::vector<xla::int64> padding,
                                  tensorflow::MirrorPadMode mode) {
  return input.CreateFrom(ir::MakeNode<ir::ops::TfMirrorPad>(
      input.GetIrValue(), std::move(padding), mode));
}

XLATensor XLATensor::tf_MirrorPadGrad(const XLATensor& grad_output,
                                      std::vector<xla::int64> input_size,
                                      std::vector<xla::int64> padding,
                                      tensorflow::MirrorPadMode mode) {
  return grad_output.CreateFrom(ir::MakeNode<ir::ops::TfMirrorPadBackward>(
      grad_output.GetIrValue(), input_size, std::move(padding), mode));
}

XLATensor XLATensor::tf_UnsortedSegmentSum(const XLATensor& data,
                                           const XLATensor& indices,
                                           xla::int64 num_segments) {
  return data.CreateFrom(ir::MakeNode<ir::ops::TfUnsortedSegmentSum>(
      data.GetIrValue(), indices.GetIrValue(), num_segments));
}
...
XLATensor XLATensor::rem(const XLATensor& input, const XLATensor& other) {
  return input.CreateFrom(ir::ops::Rem(input.GetIrValue(), other.GetIrValue()));
}
...
XLATensor XLATensor::tf_Conv(const XLATensor& input, const XLATensor& filter,
                             bool depthwise,
  ...
  return input.CreateFrom(ir::MakeNode<ir::ops::TfConv>(
      input.GetIrValue(), filter.GetIrValue(), depthwise,
      XlaHelpers::I64List(strides), padding,
      XlaHelpers::I64List(explicit_paddings), data_format,
      XlaHelpers::I64List(dilations)));
}

XLATensor XLATensor::tf_ConvBackpropFilter(
    const XLATensor& input, absl::Span<const xla::int64> filter_sizes,
    const XLATensor& out_backprop, bool depthwise,
    absl::Span<const xla::int64> strides, tensorflow::Padding padding,
    absl::Span<const xla::int64> explicit_paddings,
    tensorflow::TensorFormat data_format,
    absl::Span<const xla::int64> dilations) {
  return input.CreateFrom(ir::MakeNode<ir::ops::TfConvBackpropFilter>(
      input.GetIrValue(), XlaHelpers::I64List(filter_sizes),
      out_backprop.GetIrValue(), depthwise, XlaHelpers::I64List(strides),
      padding, XlaHelpers::I64List(explicit_paddings), data_format,
      XlaHelpers::I64List(dilations)));
}

XLATensor XLATensor::tf_ConvBackpropInput(
    absl::Span<const xla::int64> input_sizes, const XLATensor& filter,
    const XLATensor& out_backprop, bool depthwise,
    absl::Span<const xla::int64> strides, tensorflow::Padding padding,
    absl::Span<const xla::int64> explicit_paddings,
    tensorflow::TensorFormat data_format,
    absl::Span<const xla::int64> dilations) {
  return filter.CreateFrom(ir::MakeNode<ir::ops::TfConvBackpropInput>(
      XlaHelpers::I64List(input_sizes), filter.GetIrValue(),
      out_backprop.GetIrValue(), depthwise, XlaHelpers::I64List(strides),
      padding, XlaHelpers::I64List(explicit_paddings), data_format,
      XlaHelpers::I64List(dilations)));
}

XLATensor XLATensor::tf_OneHot(const XLATensor& indices,
                               const XLATensor& on_value,
                               const XLATensor& off_value, xla::int64 depth,
                               xla::int64 axis) {
  return on_value.CreateFrom(ir::MakeNode<ir::ops::TfOneHot>(
      indices.GetIrValue(), on_value.GetIrValue(), off_value.GetIrValue(),
      depth, axis));
}
...
XLATensor XLATensor::tf_StatelessRandomUniform(
    absl::Span<const xla::int64> size, const XLATensor& seeds,
    const XLATensor& minval, const XLATensor& maxval, const Device& device,
    at::ScalarType scalar_type) {
  // The Philox algorithm may cause performance regression on other devices.
  // Turn on the Philox algorithm for the CPU and GPU backends only.
  xla::Shape shape = MakeArrayShapeFromDimensions(
      ...
      MakeXlaPrimitiveType(scalar_type, &device), device.hw_type);
  return Create(
      ir::MakeNode<ir::ops::TfStatelessRandomUniform>(
          shape, seeds.GetIrValue(), minval.GetIrValue(), maxval.GetIrValue(),
          device.hw_type == swift_xla::DeviceType::TPU
              ? ir::ops::BitGeneratorType::THREE_FRY
              : ir::ops::BitGeneratorType::PHILOX),
      device, scalar_type);
...
// By value versions:
XLATensor XLATensor::diagonal_value(const XLATensor& input, xla::int64 offset,
...
XLATensor XLATensor::permute_value(const XLATensor& input,
                                   absl::Span<const xla::int64> dims) {
  return input.CreateFrom(ir::MakeNode<ir::ops::Permute>(
      input.GetIrValue(), XlaHelpers::I64List(dims)));
}

XLATensor XLATensor::resize_value(const XLATensor& input,
                                  std::vector<xla::int64> size) {
  return input.CreateFrom(
      ir::MakeNode<ir::ops::Resize>(input.GetIrValue(), std::move(size)));
}

XLATensor XLATensor::update_slice(const XLATensor& input,
                                  const XLATensor& source,
                                  absl::Span<const xla::int64> base_indices) {
  return input.CreateFrom(ir::MakeNode<ir::ops::UpdateSlice>(
      input.GetIrValue(), source.GetIrValue(), base_indices));
}

XLATensor XLATensor::physical_cast(const XLATensor& input,
                                   at::ScalarType dest_type) {
  return input.CreateFrom(
      ir::MakeNode<ir::ops::Cast>(input.GetIrValue(), dest_type),
      input.dtype());
}

XLATensor XLATensor::logical_cast(const XLATensor& input,
                                  at::ScalarType dest_type) {
  return input.CreateFrom(
      ir::MakeNode<ir::ops::Cast>(input.GetIrValue(), dest_type), dest_type);
}
...
XLATensor XLATensor::xla_pad(const XLATensor& input, at::Scalar padding_value,
                             xla::PaddingConfig padding_config) {
  return input.CreateFrom(ir::MakeNode<ir::ops::XlaPad>(
      input.GetIrValue(),
      GetIrValueForScalar(padding_value, input.shape().get().element_type(),
                          input.GetDevice()),
      std::move(padding_config)));
}

XLATensor XLATensor::xla_slice(const XLATensor& input,
                               absl::Span<const xla::int64> start_indices,
                               absl::Span<const xla::int64> limit_indices,
                               absl::Span<const xla::int64> stride) {
  return input.CreateFrom(ir::MakeNode<ir::ops::XlaSlice>(
      input.GetIrValue(), XlaHelpers::I64List(start_indices),
      XlaHelpers::I64List(limit_indices), XlaHelpers::I64List(stride)));
}
